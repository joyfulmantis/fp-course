...
tok ::
  Parser a
  -> Parser a
tok parser = parser >>= (\x -> spaces >> pure x)
...
option ::
  ...
  -> Parser a
  -> Parser a
option x parserA = parserA ||| valueParser x
...
noneof ::
  Chars
  -> Parser Char
noneof ls = satisfy (\x -> notElem x ls)
...
between ::
  ...
  -> Parser a
  -> Parser a
between parserA parserB parserC = parserA >> parserC >>= (\x -> parserB >> pure x)
...
hex ::
  Parser Char
hex = (\x -> let (Full y) = readHex x in chr y) <$> (replicateA 4 (satisfy isHexDigit))
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 parserA parserB = lift2 (:.) parserA (list (parserB >> parserA))
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby parserA parserB = sepby1 parserA parserB ||| valueParser Nil
...
eof ::
  Parser ()
eof = P helper
  where helper Nil = Result "" ()
        helper (x) = ExpectedEof x
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma x y parserA = betweenCharTok x y (sepby parserA (charTok ','))
