...
unexpectedCharParser :: Show a => Char -> Parser a
...
--- True
failed :: Show a => ParseResult a -> Parser a
failed = P . const

-- | A parser that produces zero or a positive integer.
natural :: Parser Int
natural = (list1 digit) >>= (\k -> case read k of
                                     Empty  -> failed (UnexpectedString k)
...
character = P helper
  where helper (h :. t) = Result t h
        helper Nil = UnexpectedChar ' '
...
mapParser func (P a) = P (\x -> func <$> a x)
...
bindParser func parser =
  P (\x -> onResult (parse parser x) (\i a -> parse (func a) i))
...
(>>>) parserA parserB = parserA >> parserB
...
(|||) parserA parserB = P (\x -> let parserResult = parse parserA x
                                 in if isErrorResult parserResult
                                    then parse parserB x
                                    else parserResult)
...
list ::
  Parser a
  -> Parser (List a)
list p = P $ (\x -> helper p (x, Nil))
  where helper :: Parser a -> (Input, List a) -> ParseResult (List a)
        helper parser (i, a) =
          case (parse parser i) of
            Result ii aa -> helper parser (ii, aa :. a)
            _ -> parse (valueParser (reverse a)) i
...
satisfy predicate  = character >>= (\x-> if predicate x
                                      then valueParser x
...
accBindParser :: Parser a -> Parser (List a) -> Parser (List a)
accBindParser parserX parserY = parserX >>= (\x -> (x :.) <$> parserY)
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany n parser = sequenceParser $ replicate n parser
...
phoneParser ::
  Parser Chars
phoneParser =
  lift2 (const) (lift2 (:.) digit phoneBodyParser) (is '#')
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) parserA parserB = parserA >>= (\x-> x <$> parserB)
