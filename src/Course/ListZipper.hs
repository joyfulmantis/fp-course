...
instance Functor ListZipper where
  (<$>) func (ListZipper l c r) = ListZipper (func <$> l) (func c) (func <$> r)
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper (h :. t) c r) = toList (ListZipper t h (c :. r))
toList (ListZipper Nil c r) = c :. r
...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus nc = withFocus (const nc)
...
findLeft f (ListZipper l c r) =
  case break f l of
    (_, Nil) -> IsNotZ
    (lb, (h:. t)) -> IsZ $ ListZipper t h (toList (ListZipper lb c r))
...
findRight f (ListZipper l c r) =
  case break f r of
    (_, Nil) -> IsNotZ
    (lb, (h:. t)) -> IsZ $ ListZipper (reverse (toList (ListZipper l c lb))) h t

isZor :: MaybeListZipper a -> ListZipper a -> ListZipper a
isZor (IsZ lz) _ = lz
isZor IsNotZ blz = blz
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop lz = isZor (moveLeft lz) (loop lz)
  where loop (ListZipper _ c r) = let (h :. t) = reverse (c :. r)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop lz = isZor (moveRight lz) (loop lz)
  where loop (ListZipper l c _) = let (h :.t) = reverse (c :. l)
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n lz | n < 0  = moveLeftN (n + 1) -<< moveRight lz
               | n == 0 = IsZ $ lz
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n = moveLeftN (P.negate n) 
...
moveLeftN' = helper 0
  where helper s n lz | n < 0 = case moveRight lz of
                                  IsNotZ -> Left s
                                  IsZ x -> helper (s+1) (n + 1) x
                      | n == 0 = Right lz
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth z lz@(ListZipper l _ _) = moveLeftN ((length l) - z) lz
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end lz@(ListZipper _ _ r) = let IsZ x = moveRightN (length r) lz in x
...
start ::
  ListZipper a
  -> ListZipper a
start lz@(ListZipper l _ _) = let IsZ x = moveLeftN (length l) lz in x
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft nc (ListZipper l c r) = ListZipper (c :. l) nc r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight nc (ListZipper l c r) = ListZipper l nc (c :. r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fl fc fr) (ListZipper al ac ar) =
    ListZipper (zipWith id fl al) (fc ac) (zipWith id fr ar)
...
instance Extend ListZipper where
  (<<=) func lz = ListZipper (unfoldr expandLeft lz) (func lz) (unfoldr expandRight lz)
    where expandLeft llz = (\x -> (func x,x)) <$> toOptional (moveLeft llz)
          expandRight rlz = (\x -> (func x,x)) <$> toOptional (moveRight rlz)
...
instance Traversable MaybeListZipper where
  traverse func (IsZ x) = IsZ <$> traverse func x
